# Microsoft: a dona do seu computador

Olá!  
Embora o logotipo da empresa seja uma janela com um ar de liberdade, na prática tem-se o oposto.  

1. Lobby
2. Autonomia - Windows é quem tem o controle
3. Taxa Microsoft (Windows Tax)
4. EEE - Embrace, Extend, and Extinguish
5. Venda casada
6. Hacking
7. Marketing cooperativo

- Temas relacionados


## Lobby

<img src="https://m.media-amazon.com/images/I/6148y27Qf2L._AC_UF1000,1000_QL80_.jpg" width="120" /> The Microsoft Cases (Andrew I. Gavil).  
Livro analisa o histórico processo antitruste, desde nos 90, detalhando como a corporação utilizou contratos de exclusividade com fabricantes (OEMs) para impedir a ascensão do Netscape e do Linux.




## Autonomia - Windows é quem tem o controle

📚 SO = sistema operacional
 
Quem tem controle é quem toma as decisões.


> “No Windows 10 a Microsoft, por padrão, colocou no sistema a obrigatoriedade das atualizações, porém o usuário não tem mais o poder de decidir quando vai atualizar, qual a melhor hora para atualizar, o sistema por si só atualiza e o usuário nem é consultado. O apoderamento de suas informações é feito de maneira automática, sem ao menos consultá-lo. Após coletadas, essas informações são enviadas à Microsoft e ficam armazenadas nos servidores. Observa-se a total falta de controle do usuário sobre o sistema. Não existe mais privacidade, controle sobre o sistema e segurança das informações.” [AMAN, 2018]

Ou seja, o SO Windows é um software com único dono: a Microsoft. Sendo ela a proprietária do sistema, só ela pode atualizá-lo.

Há falta de controle do usuário.

Quando o usuário, que comprou o computador e portanto é o dono, qualquer ação que o sistema execute (seja atualizar, instalar apps...) o usuário é quem concede a autorização _o que não ocorre no Windows.

⚠️ Atenção! Isto é o pilar central da segurança.



## Taxa Microsoft (Windows Tax)

Economistas e entusiastas do software livre criaram esse termo, descrevendo que, ao comprar um Dell ou HP, você paga obrigatoriamente pela licença embutida. Houve países que teve processos, onde consumidores que ganharam o direito ao reembolso do valor do software ao provarem que instalariam o Linux.




## EEE - Embrace, Extend, and Extinguish

**Abraçar, Estender e Extinguir**:  Isso foi usado internamente pela Microsoft e tornou-se público durante o famoso processo antitruste do Departamento de Justiça dos EUA contra a empresa nos anos 90.  

**Documentos Halloween**: memorandos internos da Microsoft que vazaram em 1998 e detalhavam exatamente as estratégias para interromper o avanço do software livre e do Linux usando essas técnicas.  
**Guerra dos Navegadores**: o EEE foi usado contra a Netscape (do Navigator, "avô" do Firefox).  



## Venda casada

No Brasil, juristas e economistas frequentemente citam o Código de Defesa do Consumidor para questionar por que não se pode comprar o hardware vazio. O argumento é que a Microsoft "subsidiza" as fabricantes para garantir que o Linux permaneça como uma alternativa de nicho.


## Hacking

Os "Dark Patterns", "padrões obscuros", são um tipo de *hacking* covarde. Embora um hacker (black hat) também seja
antiético ao atacar um banco ou um servidor, a Microsoft optou pelo pequeno: o usuário.
Vale lembrar que empresas financeiras ou com servidores possuem profissionais qualificados para defesa, diferente da pessoa comum. O tipo de ataque? Psicológico, num dos casos mais sujos da história da informática.

Ao vermos uma janela de *pop-up* ou propaganda, clicarmos no "xis" automaticamente para fechar a tela.  
Anos atrás, no Windows, ao invés de fechar a janela com o clique do usuário, o sistema seguia em frente fazendo a atualização. Mentira descarada, sem a menor cerimônia, típica dos canalhas.  

Vide [Dark Patterns](/ui-e-ux-dark-patterns).  



## Marketing cooperativo

A Microsoft e a Intel Inside (Intel) paga parte do marketing da fabricante em troca da exclusividade ou destaque do sistema.




## Temas relacionados

###### Acabando com outras empresas

> "Observa-se que a Microsoft, ao contrário do software livre, não estimula a livre concorrência, o desenvolvimento de outros sistemas etc. O software proprietário quer o monopólio do mercado, não estimula e nem difunde o conhecimento. Em sua a trajetória, a Microsoft acabou com as empresas que apareceram para competir nesse segmento de sistemas operacionais. Em consequência disso, muitas empresas que não conseguiram enfrentar o sistema Windows, hoje apoiam o projeto GNU/Linux."
[AMAN, 2018]  



###### Lock-in (aprisionamento do cliente)

<img src="https://upload.wikimedia.org/wikipedia/en/thumb/6/6d/Information_Rules.jpg/250px-Information_Rules.jpg" width="120" />**Information Rules** _1999; Carl Shapiro, Hal Varian (Google).  
Os autores discutem o Lock-in (aprisionamento do cliente). Eles explicam que o Windows não venceu apenas por ser "melhor", mas porque o custo de mudar para outro sistema (treinamento, compatibilidade de arquivos) é proibitivo para o usuário médio.

###### Venda de licenças: modelo de negócios

<img src="https://upload.wikimedia.org/wikipedia/en/c/c4/Cathedral-and-the-Bazaar-book-cover.jpg" width="120" />**The Cathedral and the Bazaar** (Eric S. Raymond, da OSI).  
Embora focado no código aberto, esse livro explica por que o modelo de negócios de "venda de licenças" em massa (Microsoft) conseguiu dominar o hardware comercial antes do modelo colaborativo do Linux.


###### Acordos de desconto

A lógica 'se / então':  

    se instalar em 50% das máquinas a licença custa $80, em 100% custa $10.  


Isso inviabiliza financeiramente oferecer opções sem o sistema.  

   



###### Capitalismo de Vigilância

<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/88/Shoshana_Zuboff_at_Alexander_von_Humboldt_Institut.jpg/250px-Shoshana_Zuboff_at_Alexander_von_Humboldt_Institut.jpg" width="120" />Shoshana Zuboff.
Na análise sobre o Capitalismo de Vigilância, ela discute como bigTechs moldam a infraestrutura digital para que o usuário não tenha escolha real, transformando o SO em uma "camada de controle" pré-instalada.

<img src="https://upload.wikimedia.org/wikipedia/en/4/48/The_Age_of_Surveillance_Capitalism.jpg" width="100" />
No livro ela aborda essa questão, não com termos técnicos de TI como "kernel" ou "hardware", mas focando no poder político e econômico que esse controle de infraestrutura confere às Big Techs.

> O capitalismo de vigilância "reivindica unilateralmente a experiência humana como matéria-prima gratuita para tradução em dados comportamentais.

Original...  

> *Surveillance capitalism unilaterally claims human experience as free raw material for translation into behavioral data.*"


**Destituição da Soberania**: Ela descreve isso como um "golpe vindo de cima", onde os indivíduos perdem a soberania sobre seus próprios dispositivos (o "nó") para as plataformas que controlam o SO.

**Sistemas Operacionais como Iscas**: Ela menciona que produtos como o Android não são apenas serviços, mas rotas de fornecimento de dados. O controle do SO é a garantia de que o fluxo de dados nunca pare.

**Invisibilidade do Controle**: Ela destaca que a autonomia sobre o hardware é projetada para ser invisível ao usuário, que acha que tem controle, enquanto o SO toma decisões em segundo plano.



## Microsoft e os seus "código aberto"

De uns anos para cá a empresa mudou o tom, com algumas atitudes no campo do código aberto. 

**Microsoft ElectionGuard**: É um kit de desenvolvimento (SDK) que pode rodar em dispositivos Linux simples para criar urnas que emitem comprovantes criptografados.

Mas é retórica pura, vista o Windows, de fato, ser o dono de cada um dos nossos computadores que compramos numa loja.


👉 O hardware só tem vida graças ao SO, sem isso ele é como um gato morto, exceto se extiguir com o tempo.
Como o Windows dono do hardware, ele não lhe pede licença para monitorar o que você digita, onde clica ou para onde olha. Ele simplesmente toma essa decisão porque ele controla o código base.


É isso.
\o/
