# Android + Software Livre: 40 apps pra você

Jogos, música, <em>podcast</em>, finanças, editores, navegadores... Tudo livre, sem anúncios e com liberdades.  

Muitos <em>softwares</em> que lidamos são <a href="https://joesynth.wordpress.com/2021/11/28/software-livre-e-software-proprietario/">softwares livres</a>, é o caso do Telegram ou o Firefox, por exemplo.

Eles trazem boas vantagens e por isso são usados por grandes corporações, governos; e eis porque fizemos essa lista.

Para instalação de um aplicativo como esse, uma ótima ideia é conhecer a <a href="https://joesynth.wordpress.com/2021/12/27/node-js-o/">F-Droid</a>.

A lista que se segue tem até mais de 40 aplicações, mas temos certeza que você não irá se importar com esse detalhe. O importante estar ciente desses <em>apps</em> e ter a consciência das vantagens de se utilizar um <em>soft</em> livre.

<img class="alignleft" src="https://f-droid.org/repo/org.telegram.messenger/en-US/icon_yf3k2arrpvOWv5oUZ3p1Eri1ThBaYEhH0lEWqjiyFO4=.png" width="60" />Telegram
🔗️https://telegram.org/
O app de mensagens com foco em velocidade e segurança.
Licença: basicamente GPL.

<img class="alignleft" src="https://f-droid.org/repo/icons-640/jp.co.kayo.android.localplayer.2111600304.png" width="60" />Just Player
🔗️http://justplayer-dev.blogspot.jp/
Player sem anúncios, propagandas.

<img class="alignleft" src="https://f-droid.org/repo/icons-640/com.farmerbb.notepad.112.png" width="60" />Notepad
🔗️https://github.com/farmerbb/Notepad
Anotações de forma simples. Básico, também lida com HTML ou Markdown.

<img class="alignleft" src="https://f-droid.org/repo/com.simplemobiletools.gallery.pro/en-GB/icon_w_0TfgcLygfc61MhGb3OBhXRiikrX8V0-Lxksz1BAkc=.png" width="60" />Simple Gallery Pro
🔗️https://www.simplemobiletools.com/
Organizador de arquivos profissional, recursos de visualização, edição de fotos num aplicativo fácil de usar. Navegue, gerencie, corte e edite fotos ou vídeos mais rápido do que nunca, recupere arquivos excluídos acidentalmente ou crie galerias ocultas para suas imagens e vídeos mais preciosos.

DroidRec
🔗️https://github.com/yakovlevegor/DroidRec
Gravar a tela.

Trackbook - Movement Recorder.
Grave seu movimento.

Impress Remote
Conexão remota pra fazer apresentação, via Wi-Fi ou Bluetooth.
<h5>Música e arte</h5>
Pocket Paint.

<img class="alignleft" src="https://f-droid.org/repo/org.videolan.vlc/en-US/icon_yAfSvPRJukZzMMfUzvbYqwaD1XmHXNtiPBtuPVHW-6s=.png" width="60" />VLC
🔗️https://www.videolan.org/vlc/download-android.html
Famosíssimo play de música e vídeo.

RHVoice
🔗️https://github.com/RHVoice/RHVoice/wiki
Sintetizador de voz com suporte a vários idiomas, inclusive russo.

Echo
Grava áudio em <em>background</em>.

<img class="alignleft" src="https://ftp.fau.de/fdroid/repo/icons/org.schabi.newpipelegacy.120.png" width="60" />NewPipe
🔗️https://newpipe.net/
Substitui o YouTube mas sem precisar de uma conta Google. Baixar vídeo, acelerar, são alguns recursos.
<h5>Navegador (Browser)</h5>
DuckDuckGo Privacy Browser
🔗️https://duckduckgo.com/app
Privacidade, simplificada.

Privacy Browser
🔗️https://www.stoutner.com/privacy-browser-android/
Um navegador web que respeita a sua privacidade.

Zirco Browser
🔗️https://archive.softwareheritage.org/browse/origin/http://zirco-browser.googlecode.com/svn//directory/
Navegador com bloqueador de anúncios.

Thor.
<h5>Jogos e afins</h5>
<img class="alignleft" src="https://ftp.fau.de/fdroid/repo/com.dozingcatsoftware.bouncy/en-US/icon_LCN_OECV-fBHC-WNoBRQ39REnkPq8T8w_4X9aQ5uRAc=.png" width="60" />Vector Pinball
🔗️https://github.com/dozingcat/Vector-Pinball
Pinball com gráficos vetoriais simples e ste <em>layouts</em> de mesa, usando uma biblioteca de física para simulação precisa.

<img class="alignleft" src="https://f-droid.org/repo/icons-640/com.nicobrailo.pianoli.15.png" width="60" />PianOli
🔗️https://github.com/nicolasbrailo/PianOli
App bem interessante, serve para manter seu dispositivo protegido contra toques aleatórios de um bebê. Simples!

Dolphin Emulator
🔗️https://dolphin-emu.org/
Emulador de Nitendo.

Tttris
🔗️https://archive.softwareheritage.org/browse/origin/http://tttris.googlecode.com/svn//directory/
Tetris (clone).

CrossWords
🔗️http://xwords.sourceforge.net/android.php
Um jogo semelhante ao Scrabble.

JustChess
🔗️https://gitlab.com/alaskalinuxuser/app_JustChess
Para um ou dois jogadores.

Moonlight Game Streaming
🔗️https://moonlight-stream.org/
Para jogar jogos do seu PC no Android (NVIDIA ou SunshineStream).

Gridle
🔗️https://github.com/billthefarmer/gridle
Jogo de palavras.

Gurgle
🔗️https://github.com/billthefarmer/gurgle
Jogo de palavras.

Chip8
🔗️https://github.com/dkanada/chip-emulator
Emulator Chip8.

PaddleTennis
🔗️https://gitlab.com/rogerbassons/PaddleTennisAndroid
Pong (clone).

MSnake
Clássico jogo da cobrinha.
<h5>Mercado financeiro</h5>
<img class="alignleft" src="https://f-droid.org/repo/icons-640/nitezh.ministock.80.png" width="60" />Ministocks
🔗️https://github.com/niteshpatel/ministocks
Um <em>widget</em> de preço de ações, e que usa dados financeiros provenientes do Yahoo Finance.
Nele tudança de preço (diariamente e em relação ao preço de compra), além de:
Lucros e perdas - avaliação atual e ganho total.
Alertas de preço alto e preço baixo.
Preços de câmbio.
Mudanças diárias de câmbio.
Agenda de atualização personalizada.

<img class="alignleft" src="https://f-droid.org/repo/de.cloneapps.crypto_prices/en-US/icon_xCLsDVKLGHIPsCuXV2AJGEcHtDINC37j4gQA8nBOQSI=.png" width="60" />Crypto Prices
🔗️https://gitlab.com/cl0n30/cryptoprices
Acompanhar todos os seus preços de criptomoedas favoritas.

<img class="alignleft" src="https://f-droid.org/repo/icons-640/com.brentpanther.bitcoincashwidget.23.png" width="60" />Simple Bitcoin Cash Widget
🔗️https://github.com/hwki/SimpleBitcoinWidget
Mostrar preços atuais de troca de Bitcoin Cash.

Simple Crypto Widget
🔗️https://github.com/hwki/SimpleBitcoinWidget
Exibir preços de muitas criptomoedas.

Unstoppable Wallet
🔗️https://horizontalsystems.io/
Bitcoin, Ethereum, DeFi.

Altcoin Prices
🔗️https://altcoinprices.uwot.eu/
Monitore os preços das <em>altcoins</em>.

Currencies: Exchange Rate Calculator
🔗️https://github.com/sal0max/currencies
Conversor de moedas.

nanoConverter
🔗️http://blog.zorg.tk/nanoconverter
Conversão de dinheiro.
<h5>Podcast, RSS e BitTorrent</h5>
Curator
🔗️https://github.com/abhinavmarwaha/curator
Leitor de RSS com suporte a <em>podcast</em>.

PodListen
🔗️https://github.com/einmalfel/PodListen
Leitor de <em>podcast</em> com interface leve, RSS.

Transdroid
🔗️https://www.transdroid.org/
<span class="Y2IQFc" lang="pt">Gerenciar clientes BitTorrent.</span>

News Reader
🔗️https://thesaurus.altervista.org/newsreader-help
Leitor de notícias minimalista e poderoso.
Coonversão de texto em fala, <em>feeds</em> de notícias em vários idiomas (português, francês, russo, italiano...).

LibreTorrent
🔗️https://gitlab.com/proninyaroslav/libretorrent
<em>Download</em> automático, com gerenciador Atom/RSS.
<h5>Outros</h5>
Watomatic
🔗️https://adeekshith.github.io/watomatic
Resposta automática a todas as mensagens do WhatsApp e do Facebook.

Extended emulator of МК 61/54
🔗️https://github.com/xvadim/pmk-android
Emulador de calculadoras MK, da União Soviética.

E ainda incluem: Termux, Acode.

\o/

[oes: tit !]
